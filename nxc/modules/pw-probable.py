...
class NXCModule:
    ...
    name = "pw-probable"
    description = "Brute force password based on password last set date. Often passwords are reset using this behaviour."
    ...
    def options(self, context, module_options):
        ...
        """
        self.IGNORECOMPLEX = False
        self.INCLUDECOMMON = False
        self.INCLUDEMACHINE = False
        self.PWSELECTION = False
        self.OUTFILE = False

        self.common_names = ["krbtgt", "Guest"]

        if "IGNORECOMPLEX" in module_options:
            self.IGNORECOMPLEX = True
        if "INCLUDECOMMON" in module_options:
            self.INCLUDECOMMON = True
        if "INCLUDEMACHINE" in module_options:
            self.INCLUDEMACHINE = True
        if "PWSELECTION" in module_options:
            self.PWSELECTION = module_options["PWSELECTION"].split(",")
        ...
        for account in accounts:
            if account.sAMAccountName != "" and account.pwdLastSet != "":
                if original_user == account.sAMAccountName:
                    continue
                elif self.is_machine_account(account.sAMAccountName) and not self.INCLUDEMACHINE:
                    continue
                elif self.is_common_account(account.sAMAccountName) and not self.INCLUDECOMMON:
                    continue
                elif self.is_disabled_account(account.userAccountControl):
                    continue

                context.log.debug(f"{account.sAMAccountName} password last set {account.pwdLastSet}")

                only_complex = True if self.is_complex_password_policy(passpol) and not self.IGNORECOMPLEX else False
                ...
                password_set = self.generate_password_set(account.pwdLastSet, only_complex, self.PWSELECTION)
                ...
                if password_set:
                    ...
                    for pw in password_set:
                        ...
                        if valid_user:
                            ...
                            #context.db.add_credential("plaintext", connection.domain, sAMAccountName, pw)
